...
CAT_ICONS = {
    "Finance": "💰", "Resume": "📋", "AI": "🤖", "Research": "🔬",
    "Personal": "📖", "Legal": "⚖️", "Medical": "🏥", "Other": "📁",
}
...
# ══════════════════════════════════════════════════════════════════════════════
elif page == "✏️ Try with Text":
    st.markdown("## ✏️ Try with Text")
    st.markdown("Paste any text and see how the AI classifies it.")

    samples = {
        "— pick a sample —": "",
        "Finance":  "Invoice #1234 — Amazon Web Services — Amount: $2,500 — Tax: $225 — Total: $2,725. Please process payment within 30 days.",
        "AI/ML":    "We propose a novel transformer architecture using multi-head self-attention with 12 layers and 768 hidden dimensions trained on 100 billion tokens.",
        "Medical":  "Patient presents with elevated blood pressure 145/92 mmHg and fasting glucose 118 mg/dL. CBC shows hemoglobin 11.2 g/dL. Recommend follow-up in 4 weeks.",
        "Legal":    "This Non-Disclosure Agreement is entered into between the parties. The receiving party shall hold all confidential information in strict confidence.",
        "Research": "Abstract: We investigate the effect of learning rate schedules on transformer model convergence. Results show cosine annealing reduces loss by 12% over baseline.",
    }

    choice = st.selectbox("Load a sample:", list(samples.keys()))
    text_input = st.text_area("Or type your own:", value=samples[choice],
                               height=180, placeholder="Paste document text here…")
    ...
    if st.button("🔍 Classify", type="primary", use_container_width=True):
        if not text_input.strip() or len(text_input.strip()) < 20:
            ...
            st.progress(int(min(confidence, 100)),
                        text=f"{'⚠️ Low confidence' if is_low else '✅ Confident'}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for cat in categories:
        with st.expander(f"{CAT_ICONS.get(cat, '📁')} **{cat}**"):
